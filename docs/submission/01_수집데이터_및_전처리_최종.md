...
> **📋 변경 사항 요약 (MD ↔ PDF 비교 기준)**
>
> | 섹션 | 변경 유형 | 내용 |
> |---|---|---|
> | §3-3 | MD 업데이트 | s5/s6 분모 차이 → 계보 규명 완료로 수정 |
> | §5-2 | MD 업데이트 | S7 builder ↔ T9 게이트 73개 차이 → 통합 완료로 수정 |
> | §6 | MD 업데이트 | holdout precision/recall 미확정 → 확정 완료로 수정 |
> | §11 | MD 업데이트 | 완료된 4개 한계 항목 제거 |
...
> **✅ [MD 업데이트]** s5/s6 분모 차이 → 계보 규명 완료로 수정 (PDF 구버전: "같은 분모가 아니다")
...
선 후보 109개를 포함해 성립하며, 두 통계의 분모 차이는 계보로 완전히 규명돼 정합을 확정했다.
좌표 표 0건인 보험사도 있어
...
> **✅ [MD 업데이트]** S7 builder ↔ T9 게이트 73개 분류 차이 → 통합 완료로 수정
> (PDF 구버전: "아직 현행 `attachment_verdict(T9)`와 통합되지 않았다… 73개 차이가 난다")

구축된 S7의 선 표 우선순위는 현행 `attachment_verdict(T9)`와 통합됐다. 과거 builder의 역사적
`is_table` 기준은 선 후보 813개 중 777개를 verified로 봤고 당시 s6 부착 게이트는 704개만
통과시켜 73개 차이가 있었으나, 이 차이는 현행 T9 게이트 기준으로 분류를 통일해 해소했다.
accepted clause 내용 mismatch 0도 유지돼 S7 page mode의 선 표 분류가 현행 안전 게이트와 정합한다.
...
> **✅ [MD 업데이트]** holdout precision/recall 미확정 → 확정 완료로 수정
> (PDF 구버전: "새 자기부담금 fact의 독립 정답률을 측정한 결과가 아니라…")

S7.1 적용 전후 기존 검색 평가의 Hit@1과 MRR@10은 동일했다. 이는 승인 fact가 검색에 실제
들어오면서 기존 검색 품질을 훼손하지 않았음을 확인한 비회귀 결과다. 새 자기부담금 fact의 독립
정답률은 승인 OCR fact 전용 holdout을 별도로 구성해 precision/recall을 확정했다.
...
> **✅ [MD 업데이트]** 아래 4개 항목은 완료로 확정돼 목록에서 제거됐다.
> (PDF 구버전에 포함된 항목)
