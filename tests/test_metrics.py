...
def test_tiny_llm_latency_does_not_use_a_negative_exponent():
    mx._reset_llm_metrics_for_tests()
    try:
        mx.observe_llm_call(
            provider="openai", model="test-model", purpose="term_explanation",
            outcome="success", latency_seconds=1.2762844562530518e-05,
        )
        m = mx.Metrics()
        mx._llm(m)
        line = next(
            line for line in m.render().splitlines()
            if line.startswith("llm_call_latency_seconds_sum{")
        )
        assert line.endswith(" 0.000012762844562530518")
        _parse(line)
    finally:
        mx._reset_llm_metrics_for_tests()
