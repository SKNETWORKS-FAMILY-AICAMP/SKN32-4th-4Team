"""공개 Git 저장소 경계와 데이터 없이 실행 가능한 회귀검사를 확인한다."""
...
PUBLIC_TESTS = (
    ...
    "tests/test_disability_rate.py",
    "tests/test_errors.py",
    "tests/test_evidence_precheck.py",
    "tests/test_health.py",
    "tests/test_identification_mode.py",
    "tests/test_knowledge_gap_wiring.py",
    "tests/test_llm_assessment_explainer.py",
...
SAFE_HANDOFF_HTML = {
    ...
    "19_담당자별_코드범위_및_구현현황.html",
    "20_문서확정_미확정107건_검토목록.html",
}
...
def _public_doc_allowed(rel: Path) -> bool:
    parts = rel.parts
    if rel.as_posix() == "docs/README.md":
        return True
    if len(parts) >= 3 and parts[:2] == ("docs", "handoff"):
        return len(parts) == 3 and (
            rel.suffix == ".md" or rel.name in SAFE_HANDOFF_HTML
        )
    if len(parts) == 3 and parts[:2] == ("docs", "submission"):
        return rel.suffix == ".md"
    return len(parts) >= 3 and parts[:2] == ("docs", "delivery")
...
def _check_public_boundary(files: list[Path]) -> list[str]:
    ...
    for path in files:
        ...
        if parts[0] == "docs" and not _public_doc_allowed(rel):
            errors.append(f"공개 허용 목록 밖 문서: {rel.as_posix()}")
...
def main() -> int:
    ...
    print(f"[통과] 공개 후보 파일 {len(files)}개: 금지 경로·대용량·대표 비밀키 없음")
    ...
    tests = [test for test in PUBLIC_TESTS if (ROOT / test).exists()]
    proc = subprocess.run(
        [sys.executable, "-m", "pytest", *tests, "-p", "no:randomly", "-W", "ignore"],
        cwd=ROOT,
        check=False,
    )
    if proc.returncode:
    ...
    print(f"[통과] 공개본 회귀검사 {len(tests)}개 파일")
    return 0
