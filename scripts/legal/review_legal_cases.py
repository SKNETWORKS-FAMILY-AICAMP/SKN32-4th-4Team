...
`--apply` 는 그 작업 파일에 사람이 적어 넣은 verdict 만 원장에 반영한다.

실행:
    python -m scripts.legal.review_legal_cases --queue
    # data/legal/human_review_queue.json 을 열어 각 항목의 "verdict" 와
    # "note" 를 채운다 (verdict: confirmed | corrected | rejected)
...
def main() -> int:
    ...
    if a.queue == a.apply:
        raise SystemExit("--queue 또는 --apply 중 하나만 지정하세요.")
